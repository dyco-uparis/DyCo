...
Le modèle \ed\ a été conçu sur la base de quatre constats :
...
\item Dans un monde fini, la définition d'une fonction de production doit prendre en compte l'état des ressources
...
\item Il est défini une fonction "demande", qui remplace la traditionnelle fonction de production. En effet, en présence de ressources finies, il n'est pas possible d'affirmer, à priori, que  la production sera effectivement satisfaite.
...
\item Aux quantités de ressources sont aussi associées des qualités, qui, à
...
\subsection{Paramètre de feuillet}
...
\subsection{Résistance de dissipation: R$_{P}$}
...
\begin{itemize}
\item autant de fichiers de paramétrage que nécessaire\footnote {Au minimum un feuillet énergie et un feuillet matière.}: 

\begin{itemize}
\item \textbf{world.txt} (indispensable)

\item \textbf{petrole.txt} si vous avez une feuille pétrole

\item \textbf{cuivre.txt} si vous avez une feuille cuivre
...
\item paramétré par le fichier \textit{solow.txt} (Fig. \ref{Param}). 
...
%\end{itemize}
\newpage
\section{Annexe G: Conditions de licence}

...
Le modèle a été conçu sur la base de quatre constats :


 Si le monde physique est modifié par l'application des lois de
l'économie, il n'en demeure pas moins que son évolution est gouvernée pas des
lois physiques.

 Le monde physique est fini, ceci est vrai pour toutes les ressources
matérielles et pour les ressources énergétiques fossiles.
 ...
 L'intensité de prélèvement d'une ressource est un facteur majeur
agissant sur le devenir de cette dernière.

 Si l'économie n'est pas descriptible strictement par la thermodynamique,
...
Ces trois constats fondent la structure et le fonctionnement du modèle \ed:
 ...
 Les descriptions des sphères physique et économiques du modèle
sont disjointes. Elles communiquent par l'intermédiaire des requêtes de production et la modification des capacités de l'outil de production via l'investissement et le progrès technique.

 Chaque ressource est décrite dans un feuillet qui lui est propre. La collection de feuillets ainsi obtenue est la sphère physique. Chaque feuillet quantifie la fraction utilisable et la
fraction utilisée de la ressource que l'on nommera déchet. La fraction utilisée ne peut être de nouveau utilisée qu'après recyclage.

 Il est défini une intensité de fonctionnement de l'économie qui régit
l'ensemble des feuillets physiques.

 Aux quantités de ressources sont aussi associées des qualités, qui, à
l'image des premiers et second principes de la thermodynamique, définissent,
à quantités égales, la différence de qualité d'une ressource et son état de
transformation, vers un produit ou vers un déchet.


# Structure générale<a name="Structure"></a>
...
\begin{itemize
 Son type: \textit{type : stock

 Son nom: \textit{name : cuivre

 Sa quantité totale: \textit{total stock : 500000

 Son niveau de ressource initial: \textit{Xh\_init : 500000

 Son niveau de déchet initial: \textit{Xl\_init : 0

 Sa résistance de dissipation de production~:\textit{\ Rp0 : 0.003

 Le niveau de capital initial associé a l'appareil de
production\textit{\ K0 : 1

 Ratio énergétique de recyclage (1 unité d'énergie pour recycler 1 unité
de la ressource)~:\textit{\ recyclingEnergyFlux : 1

 Utilisable comme énergie (cas du pétrole)~:\textit{\ isEnergy : False

 Taux de recyclage naturel (r=0~: pas de recyclage)~:\textit{\ r : 0

 Temps caractéristique~:\textit{\ to : 9
\end{itemize
...
\par
 ...
 La baisse des rendements en fonction de l'intensité de prélèvement, au
...
\begin{itemize
 Son type~:\textit{\ type : flow

 Son nom \textit{name : energie solaire

 Le flux incident~: \textit{incidentFlow : 1e10

 L'efficacité de conversion~: \textit{eff\_init : 0.15

 La surface installée \textit{surface\_installee : 1e-9

 Utilisable comme énergie: \textit{isEnergy : True 
...
![imageclosedloop](/images/NoyauCentral.jpg){: width="600px"}
...
\begin{itemize
 le fichier \textbf{PhysicalWorld.py

 un fichier décrivant votre sphère économique. Dans
ce manuel nous nous appuierons sur le modèle de Goodwin, décrit ans le fichier \textbf{Goodwin.py.

 le fichier \textbf{main.py
\end{itemize
\end{itemize

\begin{itemize
 autant de fichiers de paramétrage que nécessaire\footnote {Au minimum un feuillet énergie et un feuillet matière.: 

\begin{itemize
 \textbf{world.txt (indispensable)

 \textbf{petrole.txt si vous avez une feuille pétrole

 \textbf{cuivre.txt si vous avez une feuille cuivre
...
Paramétrage de la nouvelle cellule:
 ...
 pour créer une cellule stock, prendre le template StockCell.txt, rentrer
...
stockés dans les fichiers \textit{nom\_de\_la\_feuille.txt.
 ...
 Ne pas oublier d'enregistrer le fichier \textit{.txt apres modification d'une
...
tableau cell (par exemple "~cuivre~") servent ensuite a rediriger vers le
fichier "~cuivre~".txt pour l'initialisation de la feuille cuivre. Le nom du
...
de caractere que celle qui apparaît dans cell/.
 ...
 Le message \texttt{"~world successfully created~" s'affiche dans la console
...
![imageclosedloop](/images/Goodwin-txt.jpg){: width="600px"}
...
![imageclosedloop](/images/Parametrisation.jpg){: width="600px"}


Dans le fichier principal \textit{main.py, on effectue 


 le choix du fichier \textit{Solow.py comme modèle économique

 paramétré par le fichier \textit{solow.txt (Fig. \ref{Param). 

 le choix du pas de temps et de la durée de la simulation


![imageclosedloop](/images/Parametrisation2.jpg){: width="600px"}
...
![imageclosedloop](/images/ResultatSimul.jpg){: width="600px"}


![imageclosedloop](/images/ResultatSimulE.jpg){: width="600px"}
...
![imageclosedloop](/images/ResultatSimulPet.jpg){: width="600px"}
...
![imageclosedloop](/images/EcoVide-py.jpg){: width="600px"}
...
F_{HP  &  =\Pi_{HI_{P\\
F_{LP  &  =\Pi_{LI_{P+R_{PI_{P^{2\\
G  &  =F_{HP-F_{LP=\Delta\Pi I_{P+R_{PI_{P^{2%
\end{align*
\ \ \ \ \ \ \ \ 
...
\begin{itemize
 ...
 Une différence de potentiel faible (on parle de \textit{pincement des
potentiels)
...
\ \ \ \ 
\newpage
#Annexe D: Gestion du noyau central couplant les feuillets physiques: "recettes" <a name="Annexe D"></a> 
...
Cette dernière stratégie semble la plus naturelle. Cela signifie que l'on
...
énergétique effectif peut être différent du mix énergétique souhaité, par
exemple si une ressource énergétique vient a manquer.

Finalement, la sphère physique décrit l'évolution de l'état de la
ressource au cours du temps, et comptabilise les flux de matière et d'énergie
...
dans un modèle de Solow.
 ...
 Le second niveau consiste a tester diverses lois (et modèles
...
d' observer la sensibilité de la sphère physique au choix de ces lois.
 ...
 Enfin, l' utilisateur maîtrisant déjà bien le modèle est invité
...
![imageclosedloop](/images/solow2.jpg){: width="600px"}
...
![imageclosedloop](/images/Production7.jpg){: width="600px"}
...
![imageclosedloop](/images/Tableau-Bord8.jpg){: width="600px"}
...
![imageclosedloop](/images/Production9.jpg){: width="600px"}
...
![imageclosedloop](/images/Production11.jpg){: width="600px"}
...
![imageclosedloop](/images/Production13.jpg){: width="600px"}
...
\ref{Fig14c). Par conséquent, l'intensité de production maximale possible
...
Outil d'ancrage dans la réalité physique de n'importe quel modèle
macroéconomique, et observation de l'action de la sphère économique sur la
sphère physique.

Modèle développé dans un souci de simplicité et modularité afin de
faciliter son appropriation par ses utilisateurs.

Modélisation du monde physique utilisant certaines catégories de la
...
\bibitem {Boulinding1966Boulding, K.E. The economics of the coming spaceship
earth. In Environmental Quality in a Growing Economy.H. Jarrett, Ed.: 3--14.
Johns Hopkins University Press. Baltimore, 1966.

\bibitem {Roegen1971Georgescu-Roegen, N. The Entropy Law and the Economic
Process. Harvard University Press. Cambridge, MA, 1971.

\bibitem {Sollner1997Fritz Söllner, A reexamination of the role of
thermodynamics for environmental economics, Ecological Economics, Volume 22,
Issue 3, Pages 175-201, 1997.

\bibitem {Cutler1997Cutler J Cleveland, Matthias Ruth, When, where, and by
...
\bibitem {Daly1997aHerman E Daly, Georgescu-Roegen versus Solow/Stiglitz,
Ecological Economics, Volume 22, Issue 3, Pages 261-266, 1997.

\bibitem {Solow1997Robert M Solow, Georgescu-Roegen versus Solow-Stiglitz,
Ecological Economics, Volume 22, Issue 3, Pages 267-268, 1997.

\bibitem {Stiglitz1997Joseph E Stiglitz, Georgescu-Roegen versus
Solow/Stiglitz, Ecological Economics, Volume 22, Issue 3, Pages 269-270, 1997.

\bibitem {Daly1997bHerman E Daly, Reply to Solow/Stiglitz, Ecological
Economics, Volume 22, Issue 3, Pages 271-273, 1997,

\bibitem {Meadows1972Meadows, D. H., Meadows, D.H., Randers, J\o rgen, et al.
The limits to growth: a report to the club of Rome (1972). 1972.

\bibitem {Glucina2010Glucina, M. D. and Mayumi, K, Connecting thermodynamics
and economics. Annals of the New York Academy of Sciences, 1185: 11-29. (2010)

\bibitem {Godard1980Godard O., Baillon J., Céron J., Substitutions et
